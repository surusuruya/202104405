...
//       alert(parent.attr("attach-fileName")+" 파일을 삭제합니다.");
      if(!confirm(parent.attr("attach-fileName")+"파일을 삭제하시겠습니까?")){
         return false; //취소눌렀을 때 삭제막기 
      }
      
      var ano=parent.attr("attach-no");
      
      $(this).parents('li.attach-item').remove(); //remove하면 새로고침하지 않는이상 복원이 되지 않는다
                                                  //display:none은 언제든 볼 순 있다 안보일뿐
        //흔적을 남겨놓아야한다.
   ...
   $('button#addFileBtn').on('click',function(event){
      ...
      var input=$('<input>').attr({"type":"file","name":"uploadFile"}).css("disply","inline");
...
function modify_submit(){
   ...
   if($("input[name='title']").val()==""){ //제목 유효성 확인
      alert("제목은 필수입니다.");
